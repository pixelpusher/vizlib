...
let closeAnimationFrameId = null; // for animating close camera view
...
export function makeVisualiser(
  ...
  props = { title: "LivePrinter", debug: false, delay: true, 
    travelOpacity: 0.8, travelLineGlow: 0.5,
    extrudeOpacity: 0.7, extrudeLineGlow: 0.5,
    printHeadColor: 0xffff55, printHeadRadius: 20,
    fogEnabled: true, fogColor: 0x0f0f0f, fogNear: 100, fogFar: 5000,
    glowEnabled: false, glowStrength: 0.5, glowRadius: 0.15, glowThreshold: 0.1,
    minSpeed, maxSpeed,
  }
) {
  ...
  */
  ...
  const animationLine = makeAnimationLine();
  animationLine.name = 'animation-line';
  ...
  const printHeadMarker = makePrintHeadMarker({
    ...
    height: props.printHeadRadius * 2.5, // Example: cone height is 2.5 times its radius
  ...
  const camPrintHandler = {
    printEvent: async ({
      ...
      switch (type) {
        ...
        // case "travel-end":  
        {
          if (closeAnimationFrameId) {
            cancelAnimationFrame(closeAnimationFrameId);
          }
          ...
          const startPos = new Vector3(
            ...
            bedZtoScene(oldPosition.z + lp.maxz / 8)
          );
          ...
          const startLookPos = new Vector3(bedXtoScene(oldPosition.x)+ sceneWidth() / 2, bedYtoScene(oldPosition.y)+ sceneHeight() / 2, bedZtoScene(oldPosition.z));
          const endLookPos = new Vector3(bedXtoScene(newPosition.x) + sceneWidth() / 2, bedYtoScene(newPosition.y)+ sceneHeight() / 2, bedZtoScene(newPosition.z));
          ...
          const animate = () => {
            ...
            const currentLookPos = new Vector3().lerpVectors(startLookPos, endLookPos, t);
            
            closeCamera.position.copy(currentPos);
            closeCamera.lookAt(currentLookPos);
            
            if (t < 1) {
              closeAnimationFrameId = requestAnimationFrame(animate);
            } else {
  ...
  */
  export_funcs.resetAll = function() {
    // Stop animation loop
